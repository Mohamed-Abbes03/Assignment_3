...
def parse_args():
    ...
    parser.add_argument('--seed', type=int, default=10, help='seed for random number generation')
...
def main():
    ...
    if args.mode == "single":
        train_cmd = "python lib/train/run_training.py --script %s --config %s --save_dir %s --use_lmdb %d --seed %d" \
                    % (args.script, args.config, args.save_dir, args.use_lmdb, args.seed)
    elif args.mode == "multiple":
        train_cmd = "python -m torch.distributed.launch --nproc_per_node %d lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --seed %d" \
                    % (args.nproc_per_node, args.script, args.config, args.save_dir, args.use_lmdb, args.seed)
    else:
